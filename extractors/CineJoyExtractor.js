/**
 * CineJoyExtractor v5.5.0 — Zero-Backend Remote Extractor for MasterStream.
 ...
 * 1. Queries https://api.wing.st/servers for available server list
 ...
 * 4. Passes binary payload to https://api.wing.st/g
...
(function () {
  ...
  async function extract(tmdbId, imdbId, title, isTv, season, episode, year) {
    try {
      ...
      var serversRes = await fetchJson('https://api.wing.st/servers');
      ...
      for (var i = 0; i < servers.length; i++) {
        ...
        var sourceUrl = 'https://api.wing.st/?' + params.toString();
        ...
        // Step B: Post binary payload to https://api.wing.st/g
        var binaryPayload = base64url_decode(encData);
        var encryptedBytes = await fetchBinary('https://api.wing.st/g', binaryPayload);
        if (!encryptedBytes || encryptedBytes.length === 0) {
          console.warn(TAG + ' [' + server + '] wing /g returned no data');
          continue;
        }
