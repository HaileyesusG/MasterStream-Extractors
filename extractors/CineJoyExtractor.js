...
(function () {
    ...
    return new Promise(function (resolve) {
      ...
      var req = lib.get(url, { headers: { 'User-Agent': USER_AGENT } }, function (res) {
        if (res.statusCode && res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
          ...
          if (redirect.indexOf('/') === 0) redirect = 'https://cinejoy.pk' + redirect;
          return fetchTextNode(redirect).then(resolve);
        }
  ...
  async function discoverChunkNode() {
    function isExtractorChunk(code) {
      return code &&
        code.indexOf('WebAssembly') !== -1 &&
        code.indexOf('as s') !== -1 &&
        code.indexOf('as o') !== -1 &&
        code.indexOf('as n') !== -1;
    }
    ...
    if (_cachedChunk) {
      var code = await fetchTextNode('https://cinejoy.pk/' + _cachedChunk);
      if (isExtractorChunk(code)) {
        var m = code.match(/([a-zA-Z0-9_$]+)\s+as\s+s\b/);
        if (m) return { path: _cachedChunk, code: code, masterVarName: m[1] };
      }
    }
    var html = await fetchTextNode('https://cinejoy.pk/');
    ...
    var appCode = await fetchTextNode('https://cinejoy.pk/' + appMatch[0]);
    ...
    for (var j = 0; j < unique.length; j++) {
      ...
      var chunkCode = await fetchTextNode('https://cinejoy.pk/' + chunkPath);
      if (isExtractorChunk(chunkCode)) {
  ...
  function loadBoqNode() {
    ...
    _boqNodePromise = (async function () {
        ...
        return new Promise(function (resolve, reject) {
          ...
          var req = mod.request(url, {
            ...
            if (res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
              var redirect = res.headers.location;
              if (redirect.indexOf('/') === 0) redirect = u.origin + redirect;
              return nodeFetch(redirect, opts).then(resolve, reject);
            }
      ...
      var sandbox = vm.createContext({
        ...
        location: { href: 'https://cinejoy.pk/', hostname: 'cinejoy.pk', origin: 'https://cinejoy.pk', pathname: '/' },
