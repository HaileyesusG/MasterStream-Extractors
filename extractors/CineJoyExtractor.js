...
(function () {
  ...
  var K256 = new Uint32Array([
    0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5, 0x59f111f1, 0x923f82a4,
    0xd807aa98, 0x12835b01, 0x243185be, 0x550c7dc3, 0x72be5d74, 0x80deb1fe,
    0xe49b69c1, 0xefbe4786, 0x0fc19dc6, 0x240ca1cc, 0x2de92c6f, 0x4a7484aa,
    0x5cb0a9dc, 0x76f988da, 0x983e5152, 0xa831c66d, 0xb00327c8, 0xbf597fc7,
    0xc6e00bf3, 0xd5a79147, 0x06ca6351, 0x14292967, 0x27b70a85, 0x2e1b2138,
    0x4d2c6dfc, 0x53380d13, 0x650a7354, 0x766a0abb, 0x81c2c85e, 0x92722c85,
    0xa2bfe8a1, 0xa81a664b, 0xb00327c8, 0xbf597fc7, 0xc6e00bf3, 0xd5a79147,
    0x06ca6351, 0x14292967, 0x27b70a85, 0x2e1b2138, 0x4d2c6dfc, 0x53380d13,
    0x650a7354, 0x766a0abb, 0x81c2c85e, 0x92722c85, 0xa2bfe8a1, 0xa831c66d,
  ...
  function pureSha256(data) {
    ...
    for (var i = 0; i < totalLen; i += 64) {
      ...
      for (var t = 16; t < 64; t++) {
        var s0 = rotr(W[t - 15], 7)
          ^ rotr(W[t - 15], 18)
          ^ (W[t - 15] >>> 3);

        var s1 = rotr(W[t - 2], 17)
          ^ rotr(W[t - 2], 19)
          ^ (W[t - 2] >>> 10);
  ...
  function ecPointAdd(P1, P2) {
    ...
    var slope = ecMod(
      (y2 - y1) * ecModInverse(ecMod(x2 - x1, EC_P), EC_P),
      EC_P
    );
  ...
  function ecPointDouble(Pt) {
    ...
    var slope = ecMod(
      (3n * x1 * x1 + EC_A) * ecModInverse(2n * y1, EC_P),
      EC_P
    );
  ...
  function aesEncryptBlock(ptU8, keySchedule) {
    ...
    var SBOX = AES_TABLES.SBOX;
    var M0 = AES_TABLES.SUB_MIX_0;
    var M1 = AES_TABLES.SUB_MIX_1;
    var M2 = AES_TABLES.SUB_MIX_2;
    var M3 = AES_TABLES.SUB_MIX_3;
  ...
  function ghashMultiply(x, y) {
    ...
    var z0_0 = Number((z0 >> 56n) & 0xffn);
    var z0_1 = Number((z0 >> 48n) & 0xffn);
    var z0_2 = Number((z0 >> 40n) & 0xffn);
    var z0_3 = Number((z0 >> 32n) & 0xffn);

    var z0_4 = Number((z0 >> 24n) & 0xffn);
    var z0_5 = Number((z0 >> 16n) & 0xffn);
    var z0_6 = Number((z0 >> 8n) & 0xffn);
    var z0_7 = Number(z0 & 0xffn);

    var z1_0 = Number((z1 >> 56n) & 0xffn);
    var z1_1 = Number((z1 >> 48n) & 0xffn);
    var z1_2 = Number((z1 >> 40n) & 0xffn);
    var z1_3 = Number((z1 >> 32n) & 0xffn);

    var z1_4 = Number((z1 >> 24n) & 0xffn);
    var z1_5 = Number((z1 >> 16n) & 0xffn);
    var z1_6 = Number((z1 >> 8n) & 0xffn);
    var z1_7 = Number(z1 & 0xffn);
  ...
  function pureGcmDecrypt(keyU8, ivU8, ctWithTagU8, aadU8) {
    ...
    for (var j = 0; j < 16; j++) {
      expectedTag[j] = S[j] ^ encJ0[j];
    }

    var diff = 0;

    for (var j = 0; j < 16; j++) {
      diff |= tag[j] ^ expectedTag[j];
    }
  ...
  function createPureSubtle() {
    return {
      generateKey: function (algorithm, extractable, keyUsages) {
        var algName = typeof algorithm === 'string'
          ? algorithm
        ...
        if (algName === 'ECDH') {
          ...
          if (typeof crypto !== 'undefined' && crypto.getRandomValues) {
            ...
            for (var i = 0; i < 32; i++) {
              rand[i] = (Math.random() * 256) | 0;
            }
          }

          var d = ecMod(u8ToBigInt(rand), EC_N - 1n) + 1n;
          var Q = ecPointMultiply(d, {
            x: EC_Gx,
            y: EC_Gy,
          });
      ...
      importKey: function (format, keyData, algorithm, extractable, keyUsages) {
        ...
        if (algName === 'ECDH') {
          ...
          return Promise.reject(
            new Error('Invalid ECDH raw key length: ' + u8.length)
          );
        }
        ...
        return Promise.reject(
          new Error('Unsupported importKey algorithm: ' + JSON.stringify(algorithm))
        );
      },
      ...
      deriveKey: function (
        ...
        if (algName === 'HKDF') {
          ...
          if (!ikm) {
            return Promise.reject(
              new Error('Missing baseKey._ikm for HKDF')
            );
          }

          var len = (derivedKeyType.length || 256) / 8;
          var derivedBytes = pureHkdf(
            ikm,
            algorithm.salt,
            algorithm.info,
            len
        ...
        return Promise.reject(
          new Error('Unsupported deriveKey: ' + algName)
        );
      },
      ...
      deriveBits: function (algorithm, baseKey, length) {
        ...
        if (algName === 'ECDH' && algorithm.public && baseKey._d) {
          ...
          if (!peerPoint) {
            return Promise.reject(
              new Error('Missing peer public point')
            );
          }
          ...
          if (!S) {
            return Promise.reject(
              new Error('Invalid shared point (infinity)')
            );
          }
        ...
        return Promise.reject(
          new Error('Unsupported deriveBits: ' + algName)
        );
      },
      ...
      encrypt: function (algorithm, key, data) {
        ...
        return Promise.reject(
          new Error('Unsupported encrypt algorithm: ' + algName)
        );
      },
      ...
      decrypt: function (algorithm, key, data) {
        ...
        return Promise.reject(
          new Error('Unsupported decrypt algorithm: ' + algName)
        );
      },
      ...
      digest: function (algorithm, data) {
        ...
        return Promise.reject(
          new Error('Unsupported digest: ' + algName)
        );
      },
    };
  }
  ...
  function buildFingerprint() {
    ...
    return {
      ...
      dpr: (typeof window !== 'undefined' && window.devicePixelRatio)
        ...
        : 2,
      sw: (typeof window !== 'undefined' && window.screen)
        ? (window.screen.width || 390)
        : 390,
      sh: (typeof window !== 'undefined' && window.screen)
        ? (window.screen.height || 844)
        : 844,
      cd: (typeof window !== 'undefined' && window.screen)
        ? (window.screen.colorDepth || 24)
        : 24,
      tp: (typeof navigator !== 'undefined' && navigator.maxTouchPoints)
  ...
  function pureAtob(s) {
    ...
    while (i < s.length) {
      ...
      var c1 = B64_CHARS.indexOf(s[i++]);
      var c2 = i <= s.length
        ? B64_CHARS.indexOf(s[i++])
        : 64;
      var c3 = i <= s.length
        ? B64_CHARS.indexOf(s[i++])
        : 64;
      ...
      if (c2 >= 0 && c2 < 64) {
        out += String.fromCharCode(((c1 & 0xf) << 4) | (c2 >> 2));
      }

      if (c3 >= 0 && c3 < 64) {
        out += String.fromCharCode(((c2 & 0x3) << 6) | c3);
      }
    }
          ...
          res.on('end', function () {
            try {
              ...
              var clean = code
                .replace(
                  /import\{[^}]*\}from["'][^"']+["'];?/g,
                  'var X = { serverOrder: ["Lisbon", "Solara", "Athens", "Joy", "Castle", "Sakura", "Canaias"], servers: [], providers: [] };'
                )
                .replace(
                  /export\{[^}]*\};?/g,
                  ''
              ...
              var sb = vm.createContext({
                ...
                fetch: function (url, opts) {
                  return new Promise(function (fResolve, fReject) {
                    ...
                    if (opts && opts.body) {
                      var b = Buffer.isBuffer(opts.body)
                        ? opts.body
  ...
  function loadBoqEval() {
        ...
        return new Promise(function (resolve, reject) {
          ...
          xhr.onload = function () {
            ...
            resolve({
              ...
              json: function () {
                return Promise.resolve(JSON.parse(toText()));
              },

              text: function () {
                return Promise.resolve(toText());
              },
    ...
    return fetchShim(
      ...
      .then(function (res) {
        if (!res.ok) {
          throw new Error(
            'BOqDcafn download failed: HTTP ' + res.status
          );
        }
      ...
      .then(function (code) {
        ...
        try {
          ...
          console.warn(
            TAG + ' BOq eval error: ' + evalErr.message
          );
          throw evalErr;
        }

        if (!Ax && typeof g.Ax !== 'undefined') {
          Ax = g.Ax;
        }
  ...
  async function extract(
    ...
    try {
      ...
      if (!bundle || !bundle.Ax) {
        console.warn(
          TAG + ' Ax master extractor not available in BOq bundle'
        );
        return null;
      }
      ...
      var timeoutPromise = new Promise(function (_, reject) {
        setTimeout(function () {
          reject(
            new Error(
              'Ax extraction timeout (' + TIMEOUT_MS + 'ms)'
            )
          );
        }, TIMEOUT_MS);
      });
      ...
      if (!res || !res.result || !res.result.url) {
        console.warn(
          TAG
          + ' Ax returned no playable stream (failure: '
          + (res && res.failure)
          + ')'
        );
        return null;
      ...
      var captions = formatSubtitles(
        res.result.captions || res.result.subtitles || []
      );
      ...
      console.error(
        TAG + ' Extraction failed: ' + e.message
      );
      return null;
    }
  }
